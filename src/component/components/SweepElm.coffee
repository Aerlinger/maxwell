...
SweepElm::dump = ->
  CircuitElement::dump.call(this) + " " + @minF + " " + @maxF + " " + @maxV + " " + @sweepTime

SweepElm::setPoints = ->
  CircuitElement::setPoints.call this
  @lead1 = CircuitElement.interpPointPt(@point1, @point2, 1 - @circleSize / @dn)

SweepElm::draw = ->
  @setBboxPt @point1, @point2, @circleSize
  color = @setVoltageColor(@volts[0])
  CircuitElement.drawThickLinePt @point1, @lead1, color
  @setVoltageColor (if @needsHighlight() then CircuitElement.selectColor else Color.GREY)
  powerColor = @setPowerColor(false)
  xc = @point2.x1
  yc = @point2.y
  CircuitElement.drawCircle xc, yc, @circleSize
  wl = 8
  ...
  while i <= xl
    ...
    CircuitElement.drawThickLine ox, oy, xc + i, yy  unless ox is -1
    ox = xc + i
    oy = yy
    i++
...
SweepElm::getInfo = (arr) ->
  ...
  arr[4] = "range = " + CircuitElement.getUnitText(@minF, "Hz") + " .. " + CircuitElement.getUnitText(@maxF, "Hz")
  arr[5] = "time = " + CircuitElement.getUnitText(@sweepTime, "s")
