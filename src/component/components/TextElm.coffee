...
#     
TextElm::dump = ->
  CircuitElement::dump.call(this) + " " + @size + " " + @text
...
TextElm::draw = ->
  color = (if @needsHighlight() then CircuitElement.selectColor else Color.LIGHT_GREY)
  ...
  while i isnt @lines.length
    ...
    unless (@flags & TextElm.FLAG_BAR) is 0
      ...
      CircuitElement.drawLine @x1, by_, @x1 + fm.stringWidth(s) - 1, by_
...
TextElm::isCenteredText = ->
  (@flags & TextElm.FLAG_CENTER) isnt 0

TextElm::getInfo = (arr) ->
  arr[0] = @text

TextElm::getPostCount = ->
  0
