...
class Pathfinder
  ...
  validElm: (ce) ->
    (ce is @firstElm) or
    ((ce instanceof CurrentElm) and (@type is Pathfinder.INDUCT)) or
    (@type is Pathfinder.VOLTAGE and !(ce.isWire() or Util.typeOf(ce, VoltageElm))) or
    (@type is Pathfinder.SHORT and !ce.isWire()) or
    (@type is Pathfinder.CAP_V) and !(ce.isWire() or ce instanceof CapacitorElm or Util.typeOf(ce, VoltageElm))
...
#        console.log(ce + " > " + @firstElm + " >> matching " + ce + " to " + @firstElm.getCurrent())
        if Math.abs(current - @firstElm.getCurrent()) > 1e-10
          continue
